...
| XEP-0167 | Jingle RTP Sessions | 1:1 audio/video calls (`xmpp/jingle_rtp.py`): RTP `<description>` (Opus/telephone-event/VP8/H264), SSRCs, DTLS fingerprint and the Jingle RTP session lifecycle. Used together with XEP-0176/DTLS-SRTP (aiortc). |
| XEP-0176 | Jingle ICE-UDP Transport | ICE-UDP `<transport>` (ufrag/pwd/candidates) for calls; candidates and DTLS fingerprints are exchanged in `session-initiate`/`accept` and trickled via `transport-info`. |
