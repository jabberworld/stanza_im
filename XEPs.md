...
| XEP-0065 | SOCKS5 Bytestreams | Registered plugin; `discover_proxies()` autodetects the account's bytestream proxy (`category='proxy' type='bytestreams'`, see `core/discovery.py`) for the future p2p file transfer. |
...
| XEP-0363 | HTTP File Upload | Uploads one or more files to a discovered HTTP Upload service (`urn:xmpp:http:upload:0`): requests a `<slot>` (filename/size/content-type), PUTs the bytes with live per-file progress, and sends the returned `get` URL as the chat message (1:1 or MUC). Triggered from the chat toolbar, drag-and-drop of files into the chat window, and the roster "Send file → HTTP Upload" menu item. Dropping files (or picking several) opens a `FileTransferDialog` that previews images, shows a generic icon for other files, tracks per-file progress bars and lets you type one caption that is sent as a single message after the batch finishes. |
