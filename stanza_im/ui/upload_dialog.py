...
class _FileRow(QtWidgets.QWidget):
    """One file: preview/icon, name + size, and a progress bar."""
    ...
    def __init__(self, path: str, parent=None):
        ...
        size_label = QtWidgets.QLabel(format_size(_file_size(self._path)), self)
...
class FileTransferDialog(QtWidgets.QDialog):
    ...
    def set_progress(self, index: int, pct: int) -> None:
        if 0 <= index < len(self._rows):
            self._rows[index].set_progress(pct)
